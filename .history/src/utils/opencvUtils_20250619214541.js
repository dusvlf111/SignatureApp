...
export class OpenCVLoader {
  ...
  static async loadOpenCV() {
    if (this.isLoaded && this.cvInstance) return true;
    if (this.loadingPromise) return this.loadingPromise;

    this.loadingPromise = new Promise(async (resolve, reject) => {
      try {
        console.log('OpenCV WASM 로딩 시작...');
        
        // opencv-wasm 패키지 로드
        this.cvInstance = await cv();
        
        // 전역 window.cv에 할당 (기존 코드 호환성을 위해)
        window.cv = this.cvInstance;
        
        this.isLoaded = true;
        console.log('OpenCV WASM이 성공적으로 로드되었습니다.');
        resolve(true);
      } catch (error) {
        console.error('OpenCV WASM 로드 실패:', error);
        reject(error);
      }
    });
  ...
  /**
   * OpenCV 인스턴스 반환
   * @returns {Object} OpenCV 인스턴스
   */
  static getCV() {
    return this.cvInstance || window.cv;
  }
}
...
export class BackgroundRemover {
  ...
  static createContourMask(src, minArea) {
    ...
    // 유효한 윤곽선들로 마스크 생성
    const validContours = new cv.MatVector();
    ...
    if (validContours.size() > 0) {
      ...
      cv.fillPoly(mask, validContours, color);
    }
...
export class RotationCorrector {
  ...
  static correctRotation(src, options = {}) {
    ...
    cv.HoughLines(edges, lines, 1, Math.PI / 180, houghThreshold);
